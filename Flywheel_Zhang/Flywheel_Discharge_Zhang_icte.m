...
%%%%%% autodescarga i=0, circuito abierto

t1 = linspace(0,3600,3600);
z = -B1/J;
w_ad = w_ss*exp(z.*t1);
w_ad = w_ad';
v_ad = kb*w_ad;
E_ad = 0.5*J*w_ad.*w_ad;

Figad = figure;
figure (Figad);

%% COMPORTAMIENTO DE velocidad autodescarga

subplot(3,1,1)

hold on
plot(t1,w_ad,'LineWidth',1.2)
title('Velocidad angular \omega(t)');
xlabel('t [s]'); 
ylabel('\omega [rpm]');
legend('\omega(t)');
...
%% COMPORTAMIENTO DE LA tension autodescarga

subplot(3,1,2)

hold on
plot(t1,v_ad,'LineWidth',1.2)
title('Tensión V(t)');
xlabel('t[s]');
%xlim([0 60]);
ylabel('V [V]');
...
%% COMPORTAMIENTO DE LA energia autodescarga
subplot(3,1,3)

hold on
plot(t1,E_ad,'LineWidth',1.2);                 % corriente en estado estable
title('Energía de la Flywheel');
xlabel('t [s]'); 
ylabel('E [J]');
legend('E (t)');
